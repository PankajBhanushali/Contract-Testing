# Pact Contract Testing (.NET)

This repository now contains the final consolidated Consumer and Provider implementations with complete Pact contract tests (authorization + provider states). The historical step-by-step workshop content has been removed from active development paths to simplify usage.
...
pact-contract-testing/
  ...
  Provider/
    Provider.sln
    src/ (API + AuthorizationMiddleware)
    tests/ (ProductTest + provider state + auth token filter)
  pacts/ApiClient-ProductService.json
  LEARNING.md (original learning objectives reference)
  readme.md (this file)
```

Legacy step directories (`step5`–`step10`) were illustrative snapshots and are no longer required for running or extending the solution. They can be deleted safely if disk cleanliness is desired.

## What’s Implemented
- Product API: `GET /api/products`, `GET /api/products/{id}`
- Authorization: Required Bearer token (ISO timestamp <= 1 hour old). Missing/expired token → 401.
- Provider States: `products exist`, `no products exist`, `product with ID 10 exists`, `product with ID 11 does not exist`, `No auth token is provided`.
- Consumer Pact Tests: 5 interactions covering happy paths, empty list, 404, auth success, and auth failure.
- Provider Verification: Single xUnit test verifying all interactions against a real in-memory host.
...
```powershell
# Run consumer tests (generate pact)
dotnet test .\pact-contract-testing\Consumer

# Run provider verification (uses generated pact)
dotnet test .\pact-contract-testing\Provider
```

## Test Flow
1. Consumer tests spin up Pact mock server per test and write `pacts/ApiClient-ProductService.json`.
2. Provider test starts ASP.NET Core host with middleware:
   - `ProviderStateMiddleware` mutates in-memory repo per state.
   - `AuthTokenRequestFilter` refreshes auth header for dynamic timestamp.
3. Pact verifier replays all interactions, asserting status, headers, and bodies.

## Adding a New Interaction
1. Add new scenario in `Consumer/tests/ApiTest.cs` with appropriate state and matchers.
2. Run consumer tests to regenerate pact.
3. Add/adjust provider state handler in `Provider/tests/Middleware/ProviderStateMiddleware.cs` if needed.
4. Run provider tests to verify.

## Common Adjustments
- Changing auth rules: edit `AuthorizationMiddleware.cs`; update consumer header matcher pattern.
- Adding fields: extend `products` anonymous objects in consumer tests; update `Product` model & repository; adjust matchers if using flexible patterns.
- Splitting provider verification: create additional `[Fact]` methods, each scoping subsets of interactions via tag or source (optional refinement).

## CI/CD (Pact Broker Setup)

To adopt a centralized Pact Broker workflow for professional CI/CD pipelines:

1. **Start Pact Broker in Docker:**
   ```powershell
...
📖 **See [PACT-BROKER-GUIDE.md](./PACT-BROKER-GUIDE.md) for complete setup and workflow documentation.**

## Cleaning Up Legacy Steps
If you wish to physically remove the old step folders:
`step5`, `step6`, `step7`, `step8`, `step9`, `step10` may be deleted—they are superseded by the finalized `Consumer` and `Provider` projects.

## Next Steps
- Introduce Pact Broker & can-i-deploy gate.
- Add negative auth test (expired token) to increase coverage.
- Parameterize provider port & pact paths for more flexible automation.

## References
- Pact Docs: https://docs.pact.io
- Provider States: https://docs.pact.io/getting_started/provider_states
- Consumer Matchers: https://docs.pact.io/implementation_guides/net#matchers

---
This simplified README replaces the multi-step tutorial with a production-focused snapshot. See `LEARNING.md` for original educational flow.
